...
def build_keepalive_http_client(
    ...
    *,
    ...
    try:
        ...
        # transport so every SDK request lands with the model name in the
        # position the gateway will actually read. This branch also covers
        # the proxied case so the rewrite applies to proxied opencode.ai
        # traffic too. Non-opencode hosts use upstream's shared-pool path.
