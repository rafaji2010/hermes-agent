...
# Compat re-export: risk-tier overlay lives in approval_smart post-split,
# but tests and external callers import it from tools.approval.
from tools.approval_smart import _READ_ONLY_PREFIXES, _is_read_only_command, _risk_tier_verdict
...
def _smart_gate(spec: _GateSpec, command: str, description: str, pattern_key: str,
    ...
    Re-homed to :mod:`tools.approval_smart` (M13.1 risk-tier + M13.2 Shieldstral
    pre-step runs before the LLM there); this wrapper preserves the historical
    import location.
    """
    from tools.approval_smart import _smart_gate as _smart_gate_impl
    return _smart_gate_impl(spec, command, description, pattern_key, pattern_keys,
                            session_key, human_present=human_present)
