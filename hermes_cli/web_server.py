...
from hermes_cli.web_routers import (  # noqa: E402
    ...
    fleet as _fleet_routes,
    usage as _usage_routes,
)
...
# ---- FORK-OWNED RE-EXPORTS (not upstream; do not remove on compat revert) ----
# The Sep 2026 decomposition moved fork-only dashboard features out of this
# module into ``web_server_files`` / ``web_routers.{files,fleet,usage}``.
# The names below keep the ``web_server.<name>`` late-binding seam (and the
# fork's tests) working. The two mutable fleet event-store lists are shared
# BY REFERENCE; everything else resolves lazily so monkeypatching the owner
# wins at call time. No ``warn_once``: these are current homes, not aliases.
from hermes_cli.web_routers.fleet import (  # noqa: E402,F401
    _fleet_events,
    _fleet_seen,
)

_FORK_ROUTER_LAZY = {
    'fleet_events': ('hermes_cli.web_routers.fleet', 'fleet_events'),
    'fleet_models': ('hermes_cli.web_routers.fleet', 'fleet_models'),
    'fleet_models_delete_worker': ('hermes_cli.web_routers.fleet', 'fleet_models_delete_worker'),
    'fleet_models_get_worker': ('hermes_cli.web_routers.fleet', 'fleet_models_get_worker'),
    'fleet_models_set_worker': ('hermes_cli.web_routers.fleet', 'fleet_models_set_worker'),
    'fleet_run': ('hermes_cli.web_routers.fleet', 'fleet_run'),
    'fleet_status': ('hermes_cli.web_routers.fleet', 'fleet_status'),
    '_fleet_collect_events': ('hermes_cli.web_routers.fleet', '_fleet_collect_events'),
    '_fleet_fetch_all_providers': ('hermes_cli.web_routers.fleet', '_fleet_fetch_all_providers'),
    '_fleet_next_seq': ('hermes_cli.web_routers.fleet', '_fleet_next_seq'),
    # Mutable fleet-models fetch state. Reads resolve lazily to the owner
    # module (so ``ws._FLEET_MODELS_CACHE.clear()`` touches the live dict);
    # tests REBIND these names, which must target the owner module directly
    # (module __setattr__ cannot be intercepted) — see test_fleet_models_api.
    '_FLEET_MODELS_CACHE': ('hermes_cli.web_routers.fleet', '_FLEET_MODELS_CACHE'),
    '_FLEET_MODELS_AT': ('hermes_cli.web_routers.fleet', '_FLEET_MODELS_AT'),
    '_FLEET_MODELS_TASK': ('hermes_cli.web_routers.fleet', '_FLEET_MODELS_TASK'),
    # Config/session hooks the fork's tests monkeypatch on this module.
    # Routers resolve them through this seam (late-bound, call time only),
    # so a test patch on ``web_server.<name>`` is honored by the handlers.
    'load_config': ('hermes_cli.config', 'load_config'),
    'load_env': ('hermes_cli.config', 'load_env'),
    '_open_session_db_for_profile': ('hermes_cli.web_server_sessions', '_open_session_db_for_profile'),
    '_config_profile_scope': ('hermes_cli.web_server_profiles', '_config_profile_scope'),
    'get_artifact': ('hermes_cli.web_routers.files', 'get_artifact'),
    'list_artifacts': ('hermes_cli.web_routers.files', 'list_artifacts'),
    '_artifacts_dir': ('hermes_cli.web_server_files', '_artifacts_dir'),
    '_artifact_entry': ('hermes_cli.web_server_files', '_artifact_entry'),
    '_artifact_kind': ('hermes_cli.web_server_files', '_artifact_kind'),
    '_resolve_artifact': ('hermes_cli.web_server_files', '_resolve_artifact'),
    'register_artifact': ('hermes_cli.web_server_files', 'register_artifact'),
    'get_usage_budget': ('hermes_cli.web_routers.usage', 'get_usage_budget'),
    'get_usage_providers': ('hermes_cli.web_routers.usage', 'get_usage_providers'),
    '_consolidate_requests_by_model': ('hermes_cli.web_routers.usage', '_consolidate_requests_by_model'),
    '_get_commandcode_usage': ('hermes_cli.web_routers.usage', '_get_commandcode_usage'),
    '_get_opencode_usage': ('hermes_cli.web_routers.usage', '_get_opencode_usage'),
    '_get_opencode_usage_from_db': ('hermes_cli.web_routers.usage', '_get_opencode_usage_from_db'),
    '_get_openrouter_usage': ('hermes_cli.web_routers.usage', '_get_openrouter_usage'),
    '_get_usage_budget': ('hermes_cli.web_routers.usage', '_get_usage_budget'),
    '_get_usage_providers': ('hermes_cli.web_routers.usage', '_get_usage_providers'),
    '_human_delta': ('hermes_cli.web_routers.usage', '_human_delta'),
    '_parse_opencode_stats': ('hermes_cli.web_routers.usage', '_parse_opencode_stats'),
    '_short_model_name': ('hermes_cli.web_routers.usage', '_short_model_name'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    target = _FORK_ROUTER_LAZY.get(name)
    if target is not None:
        import importlib

        return getattr(importlib.import_module(target[0]), target[1])
    if name == "httpx":
        # Fork usage router (web_routers.usage) and its tests share the
        # process httpx module through this seam (monolith imported it).
        import httpx
