...
def _data_policy_guard(
    ...
    # One-time consent: model.acknowledge_data_training in config suppresses repeat warns.
    try:
        from hermes_cli.config import load_config

        acknowledged = bool((load_config().get("model") or {}).get("acknowledge_data_training"))
    except Exception:
        acknowledged = False
    warning = data_training_warning(model_name, provider=provider, base_url=base_url, acknowledged=acknowledged)
